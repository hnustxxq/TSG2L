...
class TS2Vec:
    ...
    def fit(self, data):
        ...
        max_size_dict = 0
        for i in range(len(self.size_dict)):
            if(self.size_dict[i]>max_size_dict): max_size_dict=self.size_dict[i]
        ...
        for e in range(self.count):
            ...
            print(' trian_1:','  count:  ',e,'  ', i, ' / ', self.epo, ' loss ', loss.item() / itr)
        ##################第二阶段训练##############################
        #########################################################
        self.rep=[]
        for i in range(self.count):
            ...
            for k in range(input_x.shape[0]):
                ...
                torch.cuda.empty_cache()
                if(k==0):
                    featu=repr.unsqueeze(0).detach()
                else:
                    featu=torch.cat([featu,repr.unsqueeze(0).detach()],dim=0)
    ...
    def encode(self, data):
        ...
        for i in range(self.count):
            ...
            flag=False
            if (data.shape[1] % self.size != 0):
                pad = self.size - data.shape[1] % self.size
                x_pad = torch.from_numpy(data[:,-1]).unsqueeze(1).repeat(1,pad, 1)
                input_x = torch.cat([torch.from_numpy(data), x_pad], dim=1)
                flag=True
            else:
                input_x = torch.from_numpy(data)
            temp=input_x
            input_x = input_x.reshape(input_x.shape[0],self.size, -1, input_x.shape[2]).to(torch.float).permute(0,2,1,3)


            for k in range(input_x.shape[1]):
                x = input_x[:,k]

                x=x.reshape(x.shape[0],-1,1).numpy()
                output_1, repr = self.model.parallel(x, i)
                repr = repr.reshape(-1, self.dime, repr.shape[1],repr.shape[2]).permute(2,0,1,3)
                torch.cuda.empty_cache()
                if (k == 0):
                    featu = repr.unsqueeze(1).detach()
                else:
                    featu = torch.cat([featu, repr.unsqueeze(1).detach()], dim=1)
            featu=featu.permute(0,2,1,3,4)
            featu=featu.reshape(featu.shape[0],-1,featu.shape[3],featu.shape[4])
            if(flag==True):
                self.rep.append(featu[:,0:-pad])
            else:
                self.rep.append(featu)
        max_len = self.size_1
        for index in range(0, self.rep[0].shape[1], max_len):
            ...
            flag = False
            if (index + self.size_1 > self.rep[0].shape[1]):
                pad = index + self.size_1 - self.rep[0].shape[1]
                for j in range(len(self.rep)):
                    x_pad = x_input[j][:,-1].unsqueeze(1).repeat(1,pad, 1, 1)
                    x_input[j] = torch.cat([x_input[j], x_pad], dim=1)
                flag = True

            _,_, rep = self.model_1.parallel(x_input, False)
            if(flag==True):
                rep=rep[:,0:-pad]
            if (index == 0):
                featu_1 = rep.cpu().detach().numpy()
            else:
                featu_1= np.concatenate([featu_1,rep.cpu().detach().numpy()], axis=1)
            torch.cuda.empty_cache()
